...
public class UkUserTest extends LocalTestRunnerUk {
    ...
    public void checkUsNoTaxCurrency() throws Exception {
        ...
        String price = driver.findElement(By.xpath("//a[text()='MacBook']/../following-sibling::p[@class='price']/span")).getText();
    ...
    public void checkUsTaxCart() throws Exception {
        ...
        Thread.sleep(1000); // For Presentation Only
        String pattern = "(\\d{1,3},)*\\d{1,3}\\.\\d{2}";
        String taxprice = driver.findElement(By.xpath("//ul[@class='dropdown-menu pull-right']/li/div//tbody//strong[contains(text(), 'VAT')]/../following-sibling::td")).getText();
        Pattern p = Pattern.compile(pattern);
    ...
    public void checkUsWithTaxWish() throws Exception {
        ...
        Thread.sleep(1000); // For Presentation Only
        String pattern = "(\\d{1,3},)*\\d{1,3}\\.\\d{2}";
        String price = driver.findElement(By.xpath("//table[@class='table table-bordered table-hover']//td/div[contains(text(), '$602.00')]")).getText();
        Pattern p = Pattern.compile(pattern);
        Matcher m = p.matcher(price);
        m.reset();
        while (m.find()) {
